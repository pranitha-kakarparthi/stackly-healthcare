...
async function run() {
  ...
  const list = await getJson("http://127.0.0.1:9997/json/list");
  ...
  await send("Runtime.evaluate", {
    expression:
      'document.querySelector(".care-search-row").scrollIntoView({ behavior: "instant", block: "center" })',
  });
  await delay(300);

  const shotRes = await send("Page.captureScreenshot", { format: "png" });
  const buffer = Buffer.from(shotRes.result.data, "base64");
  const outPath = path.join(artifactDir, "care_team_pills_360.png");
